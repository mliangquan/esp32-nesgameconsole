...
static uint16 chr_banks_a[8];  /* $5120-$5127: sprite CHR banks */
static uint16 chr_banks_b[4];  /* $5128-$512B: background CHR banks */
...
/* MMC5 selects sprite pattern banks from $5120-$5127 and background pattern banks
 * from $5128-$512B by tracking whether the PPU is fetching background or sprite
 * tiles. Nofrendo has a single PPU page table, so the PPU calls this before each
 * pass and we remap the pages to the right set. Only CHR mode 3 (1 KiB) is
 * implemented here, which is what this core supports. */
static void mmc5_chr_prepare(int sprite)
{
    if (sprite)
    {
        for (int i = 0; i < 8; i++)
            mmc_bankvrom(1, i * 0x400, chr_banks_a[i]);
    }
    else
    {
        for (int i = 0; i < 8; i++)
            mmc_bankvrom(1, i * 0x400, chr_banks_b[i & 3]);
    }
}

static void chr_update()
{
    /* (Re)apply the sprite banks as the default; the PPU will call chr_prepare
     * before each pass anyway. */
    mmc5_chr_prepare(1);
}
...
static void map_write(uint32 address, uint8 value)
{
    ...
    case 0x5126:
    case 0x5127:
        /* Sprite CHR banks ($5120-$5127) */
        chr_banks_a[address - 0x5120] = value | chr_upper_bits;
        break;
    ...
    case 0x512A:
    case 0x512B:
        /* Background CHR banks ($5128-$512B). These are applied to the PPU pages
         * by mmc5_chr_prepare() just before the background is rendered, so they
         * never overwrite the sprite banks. */
        chr_banks_b[address - 0x5128] = value | chr_upper_bits;
        break;
...
static void map_init(rom_t *cart)
{
    ...
    ppu_setchrprepare(mmc5_chr_prepare);
}
