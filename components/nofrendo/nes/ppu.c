...
void ppu_renderline(uint8 *bmp, int scanline, bool draw_flag)
{
   ...
   if (scanline < 240)
   {
      ...
      {
         /* MMC5 uses separate CHR banks for background and sprites */
         if (ppu.chr_prepare) ppu.chr_prepare(0);
         ppu_renderbg(vidbuf);
      }

      /* TODO: fetch obj data 1 scanline before */
      if (draw_flag && ppu.chr_prepare)
         ppu.chr_prepare(1);
