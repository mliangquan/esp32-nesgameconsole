...
/* Called by the PPU before drawing the background (sprite=0) or sprites (sprite=1).
 * MMC5 uses this to select its separate background/sprite CHR bank sets. */
typedef void (*ppu_chrprepare_t)(int sprite);
/* Called before rendering the background (sprite=false) or the sprites
 * (sprite=true), so mappers like MMC5 can select independent CHR banks for each. */
typedef void (*ppu_chrfunc_t)(bool sprite);
...
typedef struct
{
   ...
   ppu_chrprepare_t chr_prepare; // For MMC5 bg/sprite CHR bank separation
...
void ppu_setchrprepare(ppu_chrprepare_t func);
