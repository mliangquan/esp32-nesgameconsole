...
void rg_storage_init(void)
{
    ...
    if (disk_mounted)
    {
        ...
        ESP_LOGI(__FILE__, "Storage mounted at %s.", RG_STORAGE_ROOT);
    }
    else
    {
        RG_LOGE("Storage mounting failed! err=0x%x", error_code);
        ESP_LOGI(__FILE__, "Storage mounting failed! err=0x%x", error_code);
    }
}
...
int64_t rg_storage_get_free_space(const char *path)
{
	...
	esp_littlefs_info(RG_STORAGE_FLASH_PARTITION, &total, &used);
...
bool rg_storage_get_storage_info(size_t *total, size_t *used) 
{
    esp_err_t ret = esp_littlefs_info(RG_STORAGE_FLASH_PARTITION, total, used);
	return ret == ESP_OK;
}

bool rg_storage_load_rom_to_flash(const char *path)
{
    struct stat statbuf;
    if(stat(path, &statbuf) != 0)
    {
        ESP_LOGE(__FILE__, "can not stat file %s", path);
        return false;
    }

    const esp_partition_t *partition = esp_partition_find_first(ESP_PARTITION_TYPE_DATA, ESP_PARTITION_SUBTYPE_ANY, RG_NES_ROM_PARTITION_NAME);
    assert(partition != NULL);

    extern uint32_t nes_part_rom_data_size; 

    if(statbuf.st_size > nes_part_rom_data_size)
    {
        ESP_LOGE(__FILE__, "ROM size %u max than part size %u", statbuf.st_size, nes_part_rom_data_size);
        return false;
    }
    ...
    if(f == NULL)
    {
        ESP_LOGE(__FILE__, "can not open file %s", path);
        return false;
    }
    ...
    if(bytesRead < 4){
        fclose(f);
        ESP_LOGE(__FILE__, "can not read file %s", path);
        return false;
    }

    if(!rg_check_nes_header(buffer))
    {
        ESP_LOGE(__FILE__, "nes rom header check failed!");
        return false;
    }
    ...
    uint32_t crc = 0;
    size_t offs = sizeof(nes_rom_t);
    ESP_LOGI(__FILE__, "offs %d", offs);
    do{
        ESP_ERROR_CHECK(esp_partition_write(partition, offs, buffer, bytesRead));
        crc = rg_crc32(crc, buffer, bytesRead);
    ...
    rom.len = statbuf.st_size;
    rom.rom_crc = crc;

    size_t header_size = sizeof(nes_rom_t);
    ESP_ERROR_CHECK(esp_partition_write(partition, 0, (void*)&rom, header_size));

    ESP_LOGI(__FILE__, "nes rom loaded, path: %s, len: %u, crc: %x", path, rom.len, crc);
