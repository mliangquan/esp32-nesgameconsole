...
static int frameskip_phase = 2; // 0=Off, 1=4, 2=8, 3=16
...
static const char *SETTING_FRAMESKIP_PHASE = "frameskip_phase";
// --- MAIN
...
static rg_gui_event_t frameskip_phase_cb(rg_gui_option_t *option, rg_gui_event_t event)
{
    int val = frameskip_phase;
    int max = 3;

    if (event == RG_DIALOG_PREV) val = val > 0 ? val - 1 : max;
    if (event == RG_DIALOG_NEXT) val = val < max ? val + 1 : 0;

    if (val != frameskip_phase)
    {
        frameskip_phase = val;
        rg_settings_set_number(NS_APP, SETTING_FRAMESKIP_PHASE, val);
    }

    if (val == 0) strcpy(option->value, _("Off"));
    if (val == 1) strcpy(option->value, _("Every 4 frames"));
    if (val == 2) strcpy(option->value, _("Every 8 frames"));
    if (val == 3) strcpy(option->value, _("Every 16 frames"));
...
static void options_handler(rg_gui_option_t *dest)
{
    ...
    *dest++ = (rg_gui_option_t){0, _("Frameskip phase"), "-", RG_DIALOG_FLAG_NORMAL, &frameskip_phase_cb};
...
void nes_main(void)
{
    ...
    frameskip_phase = rg_settings_get_number(NS_APP, SETTING_FRAMESKIP_PHASE, 2);
    ...
    int skipFrames = 0;
    static uint8_t framePhase = 0; // 跳帧相位，避免渲染帧奇偶固定
    ...
    while (true)
    {
        ...
        if (skipFrames == 0)
        {
            ...
            {
                /* 周期性多跳 1 帧来偏移渲染帧的奇偶相位。在 30FPS 跳帧模式下，
                 * 部分游戏中，精灵会以间隔一帧交替显示/隐藏以模拟半透明效果（比如马里奥受伤时）
                 * 若渲染帧总是与闪烁的精灵的隐藏帧对齐，则精灵会持续消失。
                 * 交替相位使其呈现闪烁效果。可选项可关闭或调整
                 * 偏移频率（每 4/8/16 帧）。 */
                if (frameskip_phase > 0)
                {
                    int divisor = frameskip_phase == 1 ? 4 : frameskip_phase == 2 ? 8 : 16;
                    skipFrames = ((++framePhase) % divisor == 0) ? app->frameskip + 1 : app->frameskip;
                }
                else
                    skipFrames = app->frameskip;
            }
            else if (elapsed > frameTime + 1500 || (drawFrame && slowFrame))
            {
                /* 同上：周期性多跳 1 帧偏移渲染相位 */
                if (frameskip_phase > 0)
                {
                    int divisor = frameskip_phase == 1 ? 4 : frameskip_phase == 2 ? 8 : 16;
                    skipFrames = ((++framePhase) % divisor == 0) ? 2 : 1;
                }
                else
                    skipFrames = 1;
            }
